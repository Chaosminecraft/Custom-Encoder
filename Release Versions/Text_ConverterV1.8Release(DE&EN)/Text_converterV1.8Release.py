            ...
            try:
                if confirm=="NULL":
                    if offline=="false":
                        ...
                        if confirm.lower()=="null":
                            if version>check:
                                print(f"There is a Newer version online: {version}")
                                confirm="noupdate"
                    if offline=="true":
...
def While_true_learn_ad(): #an ad that can be turned off with settings.
    global R_NoFile
    try:
...
def main_thread(): #main thread that is running
    ...
    startup="ok"
    while True:
        if CalcError.lower()=="true":
            if lang.lower()=="en":
                ctypes.windll.user32.MessageBoxW(0, "Fraction input not suppoerted yet.", "Calculation error", 0)
                CalcError="false"
                return
            if lang.lower()=="de":
                ctypes.windll.user32.MessageBoxW(0, "Die nummer kann leider nicht gelesen werden.", "Rechenfehler", 0)
                CalcError="false"
                return
        else:
            try:
                try:
                    try:
                        ...
                        if answer.lower()=="ad setting":
                            set_ad_setting()

                        if answer.lower()=="set var":
                            R_NoLang="true"
                        ...
                        if answer.lower() == "errorhandler":#to test the errorhandler
                            print(f"there are these errors:\n1. No language file\n2. No File\n3. No Name\n4. No Floating point support yet")
                            test=input("what error? ")
                            if test.lower()=="no language" or test=="1":
                                R_NoLang="true"
                                error()
                            if test.lower()=="no file" or test=="2":
                                R_NoFile="true"
                                error()
                            if test.lower()=="no name" or test=="3":
                                R_NoName="true"
                                error()
                            if test.lower()=="no floating point support yet" or test=="4":
                                CalcError="true"
                                return
                        ...
                        if answer.lower() == 'help':#just a Boring o'l help command.
                            if lang == "en":
                                ...
                                print("The Command errorhandler shows varios selected error windows or messages.")
                                print("to get the only working link, write update.")
                                print("and there are secrets. ;)")
                                print(f"Binary is not quite done yet.\n")
                            if lang == "de":
                                ...
                                print("Der Commend My Ringtone zeigt meinen Klingelton auf meinem handy.")
                                print("Der Command errorhandler zeigt die Verfügbaren felher fenster.")
                        ...
                        if lang=="en":
                            if R_NoLang.lower()=="true":
                                ctypes.windll.user32.MessageBoxW(0, "Error, The language file 'lang.txt' couldn't be found.", "lang.txt", 0)
                                return
                            else:
                                print("Error: the Language file 'lang.txt' is not there.")
                                return
                        if lang=="de":
                            if R_NoLang.lower()=="true":
                                ctypes.windll.user32.MessageBoxW(0, "Die Datei 'lang.txt' ist nicht da.", "Datei lesefehler", 0)
                                return
                            else:
...
def error():
    ...
    global R_NoName
    if R_NoFile.lower()=="true":
        ctypes.windll.user32.MessageBoxW(0, "No name Given.", "File read error", 0)
        R_NoFile="false"
        return
    if R_NoFile.lower()=="true":
        ctypes.windll.user32.MessageBoxW(0, "Error while reading GUI errror.txt", "File read error", 0)
        R_NoFile="false"
        return
    else:
        ctypes.windll.user32.MessageBoxW(0, "Undefined", "Undefined", 0)
        return
...
def calc():
    ...
    try:
        if lang.lower()=="en":
            ...
            print("a Calculator. There is '+' '-' 'x' '/' 'mm→cm' there.")
            print(f"write help for help.\n")
        if lang.lower()=="de":
            print(f"\nein einfacher rechner für dieses Projekt.")
            print("Diese rechenoperationen sind da:\n '+' '-' '* oder x' '/' 'mm→cm'")
            print(f"schreib help für hilfe.\n")
        while True:
            if lang.lower()=="en":
                op=input("What oparation? ")
            if lang.lower()=="de":
                op=input("was wälst du aus? ")
            print()
            if op.lower()=="+":
