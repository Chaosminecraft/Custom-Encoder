    ...
    while True:
        try:
            try:
                ...
                if answer.lower()=="phex" or answer.lower()=="pbin":
                    converter()
        ...
        try:
            try:
                ...
                if answer.lower() == 'decode':
                    encode_s = input(' >> ')
                    print(f"\n", ''.join(list(text.keys())[list(text.values()).index(c)] for c in encode_s.split()), "\n")
                    reconvert()

                elif answer.lower() == 'encode':
                    decode_s = input(' >> ')
                    print(f"\n{content}:",' '.join(hexlist[c] for c in decode_s), "\n")
                    reconvert()
...
def closing():
    ...
    if now.day == fun1 and  now.month == fun:
        ...
        if lang == "en":
            ...
            print(" and decrypter. It Will be kept")
            print("for free, because i will at some time")
            print("setup an patreon site and this script")
            print("Will be Paid Via thi, but no payment")
            print("needed. It will be soon Just Optional.")
            print("so, please don't think you have to, it's")
            print("Just if you want to. so, thanks for using")
            t.sleep(15)
            exit()
        if lang == "de":
            os.system("cls")
            ...
            print("wenn etwas ist. mich würde es Freuen wenn du feedback geben würdest.")
            print("Email adresse: volkswagenfelge@gmail.com")
            t.sleep(10)
            exit()
